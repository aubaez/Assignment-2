...
void NaiveMatrixMultiply(Matrix *input0, Matrix *input1, Matrix *result)
{
    ...
    float32x4_t v0; //quadword
    float32x4_t v1; //quadword
    float32x4_t product;
    float sum;
            ...
            for(int blockRow = i; blockRow < i+blockSize; blockRow+=blockSize){ //incrementing by block size (got segfault in top loops)
                for(int blockCol = j; blockCol < j+blockSize; blockCol+=blockSize){
                    for(int k = 0; k < row1; k+=4){
                        ...
                        float vals0[4] = {input0->data[col0*blockRow + k],input0->data[col0*blockRow + (k+1)],input0->data[col0*blockRow + (k+2)],input0->data[col0*blockRow + (k+3)]};
                        float vals1[4] = {input1->data[col1*k +blockCol],input1->data[col1*(k+1) +blockCol],input1->data[col1*(k+2) +blockCol],input1->data[col1*(k+3) +blockCol]};
                        v0 = vld1q_f32(vals0);
                        v1 = vld1q_f32(vals1);
                        product = vmulq_f32(v0,v1);
                        sum = vaddvq_f32(product);
                        result->data[blockRow*col1 + blockCol] += sum;
                    }
                }
            }
