...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < rows * cols; i+=4)
    {
        ...
        //sum += host_a.data[i +2];
        //sum += host_a.data[i +3];
        float vals[4] = {host_a.data[i],host_a.data[i+1],host_a.data[i+2],host_a.data[i+3]};
        //1. Load the current 4 elements into a quadword
        v = vld1q_f32(vals);
        //2. Sum all four elements of the quadword & //3. Extract the value from the quad word
        sum += vaddvq_f32(v);
    }
